...
def fetch_ff_factors():
    def fetch(url):
        ...
        return df / 100
    ff5 = fetch("https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/F-F_Research_Data_5_Factors_2x3_daily_CSV.zip")
    mom = fetch("https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/F-F_Momentum_Factor_daily_CSV.zip")
...
def factor_attribute(result, factors):
    ...
    return {
        ...
        "r2": round(r2, 4),
        "resid_sharpe": round(float(np.mean(resid) / np.std(resid) * np.sqrt(252)), 2) if np.std(resid) > 0 else 0,
...
def make_config(tickers, enable_earnings=False, earnings_weight=0.30):
    return BacktestConfig(
        tickers=tickers, start_date="2020-01-01", rebalance_freq="monthly",
        long_threshold=0.20, short_threshold=-999.0,
        enable_regime_filter=True, vix_caution_threshold=30.0, vix_risk_off_threshold=40.0,
        enable_ic_calibration=False, enable_death_golden_cross=True,
...
def compute_cpcv_alpha(cpcv_result, group_spy):
    ...
    arr = np.array(alphas)
    return round(float(np.mean(arr)), 2), round(float(np.median(arr)), 2), round(float(np.mean(arr > 0) * 100), 1)
...
def main():
    ...
    group_spy = [(float(spy[(spy.index >= s) & (spy.index <= e)].iloc[-1]['close']) /
                  float(spy[(spy.index >= s) & (spy.index <= e)].iloc[0]['close']) - 1) * 100
                 if len(spy[(spy.index >= s) & (spy.index <= e)]) > 1 else 0 for s, e in groups]
    ...
    for w in [0.00, 0.05, 0.10, 0.15, 0.20, 0.30]:
        ...
        row = {
            "section": "weight_sweep", "label": label, "universe": "liquid_10",
            "wf_sharpe": wf.sharpe, "wf_return": wf.total_return_pct,
            "alpha_ann": fa["alpha_ann"], "alpha_t": fa["alpha_t"],
            "resid_sharpe": fa["resid_sharpe"], "r2": fa["r2"],
            "cpcv_pbo": round(cpcv.pbo * 100, 1), "cpcv_med_sharpe": cpcv.median_oos_sharpe,
            "cpcv_pct_pos": cpcv.pct_positive_oos,
            "cpcv_mean_alpha": mean_alpha, "cpcv_pct_pos_alpha": pct_pos_alpha,
        }
        all_results.append(row)
        print(f"    WF: Sharpe={wf.sharpe} Return={wf.total_return_pct}% | "
    ...
    best_row = max([r for r in all_results if r["section"] == "weight_sweep" and r["label"] != "w=0.00 (baseline)"],
                   key=lambda r: r["cpcv_med_sharpe"], default=None)
    if best_row:
    ...
    for uni_name in ["liquid_10", "liquid_20", "liquid_50"]:
        ...
        row = {
            "section": "universe_sweep", "label": label, "universe": uni_name,
            "wf_sharpe": wf.sharpe, "wf_return": wf.total_return_pct,
            "alpha_ann": fa["alpha_ann"], "alpha_t": fa["alpha_t"],
            "resid_sharpe": fa["resid_sharpe"], "r2": fa["r2"],
            "cpcv_pbo": round(cpcv.pbo * 100, 1), "cpcv_med_sharpe": cpcv.median_oos_sharpe,
    ...
    for r in all_results:
        print(f"  {r['label']:<30s} {r['wf_sharpe'] or 0:>5.2f} {r['wf_return']:>+7.1f}% "
