...
def test_run_batch_caches_signals():
    ...
    mock_model.forecast.return_value = (
        ...
        {"p10": [95 + i for i in range(10)], "p50": [100 + i for i in range(10)], "p90": [105 + i for i in range(10)]},
    )

    mock_tiingo = MagicMock()
    mock_tiingo.get_eod_history.return_value = [
        {"adjClose": float(100 + i % 10), "date": f"2024-01-{i+1:02d}"}
        for i in range(100)
    ]
    ...
    with patch.dict(sys.modules, _stub_client_modules(mock_tiingo, mock_fmp)), \
...
def test_single_failure_does_not_abort():
    ...
    def side_effect(ticker, **kwargs):
        ...
        return [{"adjClose": float(100 + i % 10), "date": f"2024-01-{i+1:02d}"} for i in range(100)]
...
def test_insufficient_data_skipped():
    mock_tiingo = MagicMock()
    mock_tiingo.get_eod_history.return_value = [
        {"adjClose": float(100 + i), "date": f"2024-01-{i+1:02d}"}
        for i in range(10)
    ]
