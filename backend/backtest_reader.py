...
def _normalize_sqlite_run(row: dict[str, Any]) -> dict[str, Any]:
    ...
    # Primary normalization now happens in `cpcv_sqlite._row_to_dict`; this
    # block is retained as a safety net for any caller that bypasses the
    # sqlite helper.
    for legacy, new in (
...
def find_runs_by_config_hash(config_hash: str, limit: int = 20) -> list[dict]:
    if supabase_backtest.is_enabled():
        try:
            ...
            logger.warning(
                "supabase find_runs_by_config_hash failed, falling back to SQLite",
                exc_info=True,
            )
